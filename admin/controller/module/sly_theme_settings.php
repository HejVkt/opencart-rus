...
class ControllerModuleSlythemeSettings extends Controller {
	...
	public function index() {   
		...
		$this->language->load('module/sly_theme_settings.php');

		//Set the title from the language file $_['heading_title'] string
		$this->document->setTitle('Sly Theme Pro Theme Options');
		...
		foreach ($text_strings as $text) {
			...
			if(isset($_POST['install_tg_other_module'])){
				...
				$dir_tg_latest = '../data_sample/themegloballite/tg_tglite_latest_module.php';
				$dir_tg_carousel = '../data_sample/themegloballite/tg_tglite_carousel_module.php';
				...
				if( is_file($dir_tg_latest) ){
					$data = mb_unserialize(file_get_contents( $dir_tg_latest ));
		
					
					$data_tg_latest = mb_unserialize(file_get_contents( $dir_tg_latest ));
		
					if( is_array($data_tg_latest) ){
						$output = array();
						$output["latest_module"] = $data_tg_latest; 
						$this->model_setting_setting->editSetting( "latest", $output );	
						$output["latest_status"] = 1; 
						$this->model_setting_setting->editSetting( "latest", $output );	
					}
				...
				if( is_file($dir_tg_carousel) ){
					$data = mb_unserialize(file_get_contents( $dir_tg_carousel ));
		
					
					$data_tg_carousel = mb_unserialize(file_get_contents( $dir_tg_carousel ));
		
					if( is_array($data_tg_carousel) ){
						$output = array();
						$output["carousel_module"] = $data_tg_carousel; 
						$this->model_setting_setting->editSetting( "carousel", $output );	
						$output["carousel_status"] = 1; 
						$this->model_setting_setting->editSetting( "carousel", $output );	
					}
					...
					$query = $this->db->query("
							...
							(22, 1, 'latest.f94dzl7kdil', 'content_top', 1),
							(23, 1, 'carousel.wargaznt49c', 'content_bottom', 0),
							(24, 3, 'category', 'column_left', 0),
							(25, 3, 'latest.z1g88hc850p', 'column_left', 1),
							(26, 3, 'information', 'column_left', 2),
							(28, 2, 'category', 'column_left', 0),
